# OpenBittube - a fully open sourced implementation of Bittube backend

In this example [restbed](https://github.com/Corvusoft/restbed/) is used to
demonstrate how to provide Bittube related JSON REST service. For this purpose,
 a service called Open Bittube was developed.


OpenBittube is an open source implementation of backend of
https://coin.bit.tube/. The frontend, which includes HTML, CSS, JavaScript, was adapted
from (and originally developed by) https://coin.bit.tube/.

Open Bittube's backend is open sourced, free
to use, host and modify. Additionally, some features were added/changed as compared
to Bittube. They include:

 - google analytics, cloudflare, images and flash were removed.
 - frontend's html and javascript upgrades and cleanups.
 - transaction service fee was set to zero (same as in Bittube).
 - the wallets generated use 25 word mnemonics, fully compatible with official bittube wallets
(13 word mnemonics generated by Bittube work as usual though).
...
This is OpenBittube running on stagnet network. You can use it to play around with it.
...
The Open Bittube consists of four components that need to be setup for it to work:
 ...
 - Frontend - it is virtually same as that of Bittube, except before mentioned differences.
  It consists of HTML, CSS, and JavaScript.
 - Bittube daemon - daemon must be running and fully sync, as this is
 where all transaction data is fetched from and used. Daemon also commits txs
 from the Open Bittube into the Bittube network.
 - Backend - fully written in C++. It uses [restbed](https://github.com/Corvusoft/restbed/) to serve JSON REST to the frontend
 and [mysql++](http://www.tangentsoft.net/mysql++/) to interface the database. It also accesses Bittube blockchain and "talks"
 with Bittube deamon.
...
Open Bittube is not as fast as Bittube.
...
Below are example and basic instructions on how to setup up and run Open Bittube on Ubuntu 16.04.
For other Linux operating systems, the instructions are analogical.


#### Bittube download and compilation
...
git clone --recursive https://github.com/ipbc-dev/bittube-light-wallet

cd bittube/

# checkout last bittube version
git checkout -b last_release v0.12.0.0

make
```

#### Compilation of the OpenBittube (don't run it yet)

Once Bittube was downloaded and compiled, we can download Open Bittube and compile it.
...
work without database, setup frontend, and synced and running bittube blockchain.

```bash
# need mysql++ libraries
sudo apt install libmysql++-dev

# go to home folder if still in ~/bittube-light-wallet
cd ~

git clone https://github.com/ipbc/bittube-light-wallet.git
...
# altearnatively can use cmake -DMONERO_DIR=/path/to/bittube-light-wallet_folder ..
# if bittube is not in ~/bittube-light-wallet
...
Download `bittube.sql` provided and setup the `bittube` database. `bittube.sql` script will
drop current `bittube` if exist. So don't run it, if you have already some important information
in the `bittube` database.

Assuming we are still in `build` folder:

```bash
# apply it to mysql
mysql -p -u root < ../sql/bittube-light-wallet.sql
```
...
```bash
sudo mkdir /var/www/html/bittube-light-wallet
sudo cp -rvf ../html/* /var/www/html/bittube-light-wallet/
```

Setup document root in `lighttpd.conf` into bittube folder
...
```bash
server.document-root    = "/var/www/html/bittube-light-wallet"
```
...
Before running `bittube`:
 ...
 - make sure bittube daemon is running and fully sync. If using testnet or stagenet networks, use bittube daemon
 with `--testnet` or `--stagenet` flags!


To start for mainnet:
```bash
./bittube-light-wallet
```

To start for testnet:
```bash
./bittube-light-wallet -t
```

To start for stagenet:
```bash
./bittube-light-wallet -s
```

To start for stagenet with non-default location of `config.json` file:

```bash
./bittube-light-wallet -s -c /path/to/config.json
```


## OpenBittube JSON REST API

Example JSON REST requests and their responses of [OpenBittube](https://github.com/ipbc/bittube-light-wallet) are provided below. The long term goal
...
the OpenBittube api does not fully conform to that.

#### get_version

Get version of the OpenBittube, its API and bittube.
...
```json
{
  ...
  "bittube_version_full": "0.10.3.1-125f823"
}
```
...
Login an existing or new user into OpenBittube.
...
Get random outputs from the bittube daemon to be used as ring members
...
Submit raw tx in hex generated by the fronted to be relayed to bittube network.
...
Other examples can be found on  [github](https://github.com/ipbc?tab=repositories).
